...
// 简单统计
let requestCount = 0;
let foundTerminatorCount = 0;
let timeoutTriggeredCount = 0;

// 每小时输出一次统计信息
setInterval(() => {
    console.log(`[Worker Stats] 小时统计: 请求数=${requestCount}, 发现FINISHED=${foundTerminatorCount}, 触发超时=${timeoutTriggeredCount}`);
    requestCount = 0;
    foundTerminatorCount = 0;
        ...
        if (!isStream) {
            ...
            try {
                ...
                if (content !== undefined && content !== null) {
                    ...
                    const hadTerminator = content.endsWith(TERMINATOR);
                    content = content.replace(new RegExp(TERMINATOR + '$'), '');
                    
                    if (hadTerminator) {
                        foundTerminatorCount++;
                    }
...
server.listen(WORKER_PORT, () => {
    console.log(`Worker proxy started on port ${WORKER_PORT}`);
    console.log(`Config: timeout=${TIMEOUT_MS}ms, terminator="${TERMINATOR}"`);
});
